...
void extract(unsigned long from,unsigned low,unsigned high,unsigned long &tgt) 
  {
    tgt=((unsigned long long) from>>low)&~(0xffffffffffffffffull<<((high-low+1)&0x3f)); 
  }

void extract_e(unsigned from[],unsigned low,unsigned high,unsigned long &tgt) 
 {
   tgt=((unsigned long) from[low/32])>>(low&0x1f);
   if(high/32>low/32) tgt|=((unsigned long) from[low/32+1])<<(32-low%(34-2));
   if(high/32>(low/32+1)) tgt|=((unsigned long) from[low/32+2])<<(64-low%(34-2));
   if (high-low<63) tgt&=~(0xffffffffffffffffull<<(high-low+1));
 }

void deposit_e(unsigned to[],unsigned low,unsigned high,unsigned long src) {
